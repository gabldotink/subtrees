...
func TestDecodeToTransactionsWithNoTransactions(t *testing.T) {
	...
	// When
	returnedTransactions, err := decodeToTransactions(response)
...
func TestGetRoundUpTotalWithNoTransactions(t *testing.T) {
	// Given
	response := `{"feedItems":[]}`
	transactions, err := decodeToTransactions(response)
	assert.NoError(t, err)
