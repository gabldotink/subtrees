...
func TestGetRoundUpOperationsWithNoTransactions(t *testing.T) {
	// Given
	transactionsJSON := `{"feedItems":[]}`
	transactions, err := decodeToTransactions([]byte(transactionsJSON))
	assert.NoError(t, err)
	expectedRoundUpOperations := []RoundUpOperation{}

	// When
	returnedRoundUpOperations := getRoundUpOperations(transactions)

	// Then
	assert.Equal(t, expectedRoundUpOperations, returnedRoundUpOperations)
}

func TestGetRoundUpOperationsWithOneTransaction(t *testing.T) {
	// Given
	transactionsJSON := `{"feedItems":[{"feedItemUid":"feed-1","categoryUid":"category-1","amount":{"currency":"GBP","minorUnits":617},"sourceAmount":{"currency":"GBP","minorUnits":617},"direction":"OUT","updatedAt":"2022-10-11T16:59:39.153Z","transactionTime":"2022-10-11T16:59:38.526Z","settlementTime":"2022-10-11T16:59:39.029Z","source":"FASTER_PAYMENTS_OUT","status":"SETTLED","transactingApplicationUserUid":"transacting-application-user-1","counterPartyType":"PAYEE","counterPartyUid":"counter-party-1","counterPartyName":"Mickey Mouse","counterPartySubEntityUid":"counter-party-sub-entity-1","counterPartySubEntityName":"UK account","counterPartySubEntityIdentifier":"204514","counterPartySubEntitySubIdentifier":"00000825","reference":"External Payment","country":"GB","spendingCategory":"PAYMENTS","hasAttachment":false,"hasReceipt":false,"batchPaymentDetails":null}]}`
	transactions, err := decodeToTransactions([]byte(transactionsJSON))
	assert.NoError(t, err)
	expectedRoundUpOperations := []RoundUpOperation{
		RoundUpOperation{Uid: "feed-1", Currency: "GBP", Value: 83},
	}
...
func TestGetRoundUpOperationsWithMultipleTransactions(t *testing.T) {
	// Given
	...
	assert.NoError(t, err)
	expectedRoundUpOperations := []RoundUpOperation{
		RoundUpOperation{Uid: "feed-9", Currency: "GBP", Value: 83},
		RoundUpOperation{Uid: "feed-10", Currency: "GBP", Value: 62},
		RoundUpOperation{Uid: "feed-11", Currency: "GBP", Value: 63},
		RoundUpOperation{Uid: "feed-12", Currency: "GBP", Value: 10},
		RoundUpOperation{Uid: "feed-13", Currency: "GBP", Value: 72},
		RoundUpOperation{Uid: "feed-14", Currency: "GBP", Value: 37},
	}
...
func TestGetRoundUpOperationsWithOneUnroundableTransaction(t *testing.T) {
	// Given
	transactionsJSON := `{"feedItems":[{"feedItemUid":"feed-1","categoryUid":"category-1","amount":{"currency":"GBP","minorUnits":600},"sourceAmount":{"currency":"GBP","minorUnits":600},"direction":"OUT","updatedAt":"2022-10-11T16:59:39.153Z","transactionTime":"2022-10-11T16:59:38.526Z","settlementTime":"2022-10-11T16:59:39.029Z","source":"FASTER_PAYMENTS_OUT","status":"SETTLED","transactingApplicationUserUid":"transacting-application-user-1","counterPartyType":"PAYEE","counterPartyUid":"counter-party-1","counterPartyName":"Mickey Mouse","counterPartySubEntityUid":"counter-party-sub-entity-1","counterPartySubEntityName":"UK account","counterPartySubEntityIdentifier":"204514","counterPartySubEntitySubIdentifier":"00000825","reference":"External Payment","country":"GB","spendingCategory":"PAYMENTS","hasAttachment":false,"hasReceipt":false,"batchPaymentDetails":null}]}`
	transactions, err := decodeToTransactions([]byte(transactionsJSON))
	assert.NoError(t, err)
	expectedRoundUpOperations := []RoundUpOperation{}
